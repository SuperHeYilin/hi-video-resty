          ...
          if (event.getType().equals(CacheEvent.CacheEventType.ALL)) {
            ...
            do {
              ...
              if (!rawKeys.isEmpty()) {
                ...
                byte[][] groupedRawKeys = groupedKeys.toArray(new byte[groupedKeys.size()][]);
                for (byte[] bs : groupedRawKeys) {
                	jedis.del(new String(bs,"UTF-8"));
				}
              }
            } while (!finished);
//            RedisManager.deleteKeyByPrefix(event.getGroup() + Constant.CONNECTOR);
              jedis.del(groupKeys);
          }
        }
      }
  ...
  private void addGroupKey(Object jedis, String group, String key, int expired) {
    ...
    if (jedis instanceof ShardedJedis) {
      ((ShardedJedis) jedis).zadd(groupRawKeys, expired > 0 ? System.currentTimeMillis() + expired * 1000L : expired,key.getBytes());
    } else if (jedis instanceof Jedis) {
      ((Jedis) jedis).zadd(groupRawKeys, expired > 0 ? System.currentTimeMillis() + expired * 1000L : expired, key.getBytes());
    }
  }
  ...
  private byte[] getGroupedKey(byte[] groupRawKey, byte[] rawKey) {
    ...
    byte[] regionedKey = Arrays.copyOf(connectedRawKey, connectedRawKey.length + rawKey.length);
    System.arraycopy(rawKey, 0, regionedKey, connectedRawKey.length, rawKey.length);
    return regionedKey;
  }
