	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
		...
		if (!isExclusion(request.getRestPath())) {
			try {
				...
				SecurityManager securityManager=new MySecurityManager(context);
				PtPermissionData pp=securityManager.getNowRouteData();
				if(pp!=null){
					if(pp.get("param_name")==null){
						request.SetQueryParams("auth_data",pp.get("data_value"));
					}else{
						request.SetQueryParams(pp.get("param_name").toString(),pp.get("data_value"));
					}
				}
